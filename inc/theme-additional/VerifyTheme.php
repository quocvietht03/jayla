...
// https://themeforest.net/item/jayla-minimal-modern-multiconcept-woocommerce-theme/22486296
define("ITEM_ID","22486296"); // Replace it by current item id
...
if (!class_exists('BearsthemesCommunicator')):
  class BearsthemesCommunicator extends BearsthemesAPI {
    ...
    function __construct($baseUrl='https://api.bearsthemespremium.com/') {
        parent::__construct($baseUrl);
    }
...
class VerifyTheme {
     ...
     */
    public function purchase_code_validate( $code = '' ) {
        $envato = new EnvatoMarket();
        $envato->setAPIKey( TOKEN_KEY );

        return $envato->isPurchaseCodeLegit( $code );
    }
     ...
     * @return [array]
     */
    public function bearsserver_purchase_code_is_used_validate( $code = '' ) {
        $communicator = new BearsthemesCommunicator();
        $already_in_use = ! isInstallationLegit( array(
            'purchase_code' => $code
        ) );
        ...
        if( ! $already_in_use ) {
            $server_name = empty($_SERVER['SERVER_NAME']) ? $_SERVER['HTTP_HOST']: $_SERVER['SERVER_NAME'];
            if( !isLocalhost($server_name) ) {
                $communicator->unRegisterDomains( $code );
                $communicator->registerDomain( $code, $server_name );
            }

            // save purchase_code
            update_option( 'verifytheme_settings', array(
                'purchase_code' => $code,
            ) );

            return array(
                'message' => 'Theme activate license successful, thank you!',
                'st' => 'success',
            );
        } else {
            $connected_domain = $communicator->getConnectedDomains( $code );
            $link_purchase = 'http://themeforest.net/cart/add_items?ref=bearsthemes&item_ids=' . ITEM_ID;
            return array(
                'message' => implode( array(
                    'Purchase code is used on another domain, Are you using this theme for a new site?',
                    'Please',
                    '<a href="'. $link_purchase .'" target="_blank">BUY new license</a>',
                    '. Thank you so much!',
                ) ),
                'st' => 'error',
            );
     ...
     * @return [array]
     */
    public function main_purchase_code_validate( $code = '' ) {
        /* Step 1: validate purchase code ready */
        $purchase_code_validate = $this->purchase_code_validate( $code );
        if( true != $purchase_code_validate ) {
            return array(
                'message' => 'Invalid purchase code, please try again!',
                'st' => 'error'
            );
        }

        /* Step 2: validate purchase code on server Bearsthemes */
        $bearsserver_purchase_code_validate = $this->bearsserver_purchase_code_is_used_validate( $code );
        return $bearsserver_purchase_code_validate;
    }
